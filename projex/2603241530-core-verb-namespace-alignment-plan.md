...
- [ ] Every core verb driver's `Namespace` property matches its spec group (e.g., `"core.var"`, `"core.flow"`)
- [ ] Directory structure mirrors spec groups: `Var/`, `Eval/`, `Flow/`, `Nav/`, `Collection/`, `Math/`, `Store/`, `Channel/`, `Signal/`, `Error/`, `Debug/`
- [ ] C# namespace declarations match directory paths (`Zoh.Runtime.Verbs.{Group}`)
- [ ] `VerbRegistry.RegisterCoreVerbs()` compiles with updated references
- [ ] `dotnet build` succeeds
- [ ] `dotnet test` passes (all existing tests)
- [ ] Suffix resolution still works: `/set;`, `/write;`, `/sleep;` etc. resolve correctly
