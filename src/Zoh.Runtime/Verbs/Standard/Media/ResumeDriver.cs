...
public class ResumeDriver : IVerbDriver
{
    ...
    public DriverResult Execute(IExecutionContext context, VerbCallAst call)
    {
        ...
        if (ctx == null) return DriverResult.Complete.Fatal(new Diagnostics.Diagnostic(Diagnostics.DiagnosticSeverity.Fatal, "invalid_context", "Resume requires a valid Context.", call.Start));
