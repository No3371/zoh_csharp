...
public class PauseDriver : IVerbDriver
{
    ...
    public DriverResult Execute(IExecutionContext context, VerbCallAst call)
    {
        ...
        if (ctx == null) return DriverResult.Complete.Fatal(new Diagnostics.Diagnostic(Diagnostics.DiagnosticSeverity.Fatal, "invalid_context", "Pause requires a valid Context.", call.Start));
