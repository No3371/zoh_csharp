...
public class ConverseValidator : IVerbValidator
{
    public string VerbName => "converse";
    ...
    public IReadOnlyList<Diagnostic> Validate(VerbCallAst call, CompiledStory story)
    {
        ...
        if (call.UnnamedParams.Length < 1)
        {
            diagnostics.Add(new Diagnostic(
                ...
                "missing_parameter",
                $"/converse requires at least 1 parameter (the content).",
                call.Start,
                story.Name
            ));
        }
        ...
        if (call.NamedParams.TryGetValue("timeout", out var timeoutVal))
        {
            if (timeoutVal is ValueAst.String || timeoutVal is ValueAst.Boolean)
            {
                diagnostics.Add(new Diagnostic(
                    ...
                    "invalid_type",
                    $"The 'timeout' parameter for /converse must be numeric if provided as a literal.",
                    call.Start,
                    story.Name
                ));
            }
